...
const Popup: React.FC<PopupProps> = ({
  ...
  return (
    <>
      <div
        className={`fixed top-0 left-0 w-full h-full bg-black opacity-40 z-${zindex}`}
      ></div>
      <section
        className={`fixed left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[${widthh}%] max-h-[90%] overflow-auto z-${zindex2} rounded-xl hide-scrollbar`}
      >
        <div className={`relative z-${zindex2}`}>
          <div className=" w-full rounded-lg h-auto backdrop-blur-lg bg-white border shadow-lg flex flex-col  p-5">
            <h2 className={`text-secondary text-sub_head font-semibold`}>
              {title}
            </h2>
