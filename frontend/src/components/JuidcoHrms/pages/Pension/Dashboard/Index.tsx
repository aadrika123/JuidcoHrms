...
const Dashboard = () => {
  ...
  // Count for "Process initiated by HR" Pie Chart
  const approvedCount = empPensions?.data?.filter((emp: any) => emp.isProcessed === true)?.length || 0;
  const rejectedCount = empPensions?.data?.filter((emp: any) => emp.isProcessed === false)?.length || 0;


  // Count for "Disbursement" Pie Chart
  const processedCount = empPensions?.data?.filter((emp: any) => emp.progress === 6)?.length || 0;
  const processingCount = empPensions?.data?.filter((emp: any) => emp.progress !== 6)?.length || 0;
  // Chart options
  ...
  const chartOptions2 = {
    ...
    series: [processedCount, processingCount],
    labels: ["Processed", "Processing"],
    colors: ["#665DD9", "#3592FF"],
    dataLabels: {
      enabled: false,
    },
