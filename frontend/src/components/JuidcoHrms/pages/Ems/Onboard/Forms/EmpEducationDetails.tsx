  ...
  return (
    <div>
      ...
      {/* <Formik
        ...
        onSubmit={saveDataToSessionStorage}
      >
        {(formikProps) => (
          <Form>
            <div className="mt-4">
              <table className="table table-md">
                ...
                <tbody>
                  {formikProps.values.education.map((row, rowIndex) => (
                    <tr key={rowIndex}>
                      {COLUMNS_FOR_EDUCATION.map((column, colIndex) => (
                        <td key={colIndex}>
                          {column.ACCESSOR === "edu_level" ? (
                            <span>{row.edu_level}</span>
                          ) : (
                            <Field
                              type="text"
                              name={`education[${rowIndex}].${column.ACCESSOR}`}
                              placeholder={`Enter ${column.HEADER}`}
                              className="w-full h-full p-2 bg-transparent outline-none"
                            />
                          )}
              ...
              <div className="w-full flex items-center justify-end mt-3">
                <Button
                  ...
                  onClick={() =>
                    formikProps.setFieldValue("education", [...formikProps.values.education, { edu_level: "" }])
                  }
                >
                  + Add More
                </Button>
